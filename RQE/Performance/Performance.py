...
class Performance:
    ...
    def find_min_rank(self,model=DecisionTreeRegressor(), bracket=10):
        ...
        sorted_index = [i[0] for i in sorted(enumerate(self.test_dependent), key=lambda x:x[1], reverse=True)]
        ...
        predicted_sorted = sorted(actual_ranks, key=lambda x: x[-1], reverse=True)
